        ...
        function collectCanvasState() {
            const mode = getLayoutMode();
            const previousState = normalizeCanvasState(lastCanvasState || { elements: [] });
            const previousMap = new Map(previousState.elements.map((item) => [item.id, item]));
            const elements = [];
            (contentLayer || canvas).querySelectorAll('.canvas-element').forEach(el => {
                const elementId = ensureElementId(el);
                const type = (el.className.match(/el-(\w+)/) || [])[1] || 'unknown';
                const existing = previousMap.get(elementId);
                const normalizedExisting = existing ? normalizeElementRecord(existing) : null;
                const currentLayout = toLayoutSnapshotFromElement(el);
                const layouts = normalizedExisting
                    ? {
                        [LAYOUT_DESKTOP]: normalizeLayout(normalizedExisting.layouts[LAYOUT_DESKTOP], currentLayout),
                        [LAYOUT_MOBILE]: normalizeLayout(normalizedExisting.layouts[LAYOUT_MOBILE], currentLayout)
                    }
                    : {
                        [LAYOUT_DESKTOP]: normalizeLayout(currentLayout),
                        [LAYOUT_MOBILE]: normalizeLayout(currentLayout)
                    };
                layouts[mode] = normalizeLayout(currentLayout);

                elements.push({
                    id: elementId,
                    type,
                    left: layouts[mode].left,
                    top: layouts[mode].top,
                    width: layouts[mode].width,
                    height: layouts[mode].height,
                    zIndex: layouts[mode].zIndex,
                    rotation: layouts[mode].rotation,
                    transform: layouts[mode].transform,
                    letterText: el.dataset.letterText || '',
                    letterBg: el.dataset.letterBg || '#ff69b4',
                    letterFontSize: el.dataset.letterFontSize || 'medium',
                    letterTextAlign: el.dataset.letterTextAlign || 'center',
                    layouts,
                    contentHTML: Array.from(el.children)
                        .filter(c => !c.classList.contains('resize-handle') && !c.classList.contains('rotate-handle') &&
                            !c.classList.contains('context-bar') && !c.classList.contains('media-close-btn') &&
            ...
            const nextState = {
                elements,
                footerText: footerTextEl ? footerTextEl.innerText : '',
                bgColor: document.body.style.background
            };
            lastCanvasState = cloneDeep(nextState);
            return nextState;
        }
        ...
        function saveCanvas(immediate = false, securityConfig = null) {
            clearTimeout(saveTimeout);
            if (immediate) {
                saveCanvasNow(securityConfig).catch((error) => {
                    console.warn('Background save failed:', (error && error.message) || error);
                });
            } else {
                saveTimeout = setTimeout(() => {
                    saveCanvasNow().catch((error) => {
                        console.warn('Background save failed:', (error && error.message) || error);
                    });
                }, 500);
            }
        ...
        async function saveCanvasNow(securityConfig = null) {
            ...
            try {
                await saveCanvasToIndexedDb(canvasId, state);
            } catch (err) {
                console.warn('IndexedDB save failed:', (err && err.message) || err);
            }

            try { localStorage.setItem(localCanvasKey, JSON.stringify(elements)); } catch (e) { }
            ...
            try {
                ...
                console.warn('Supabase save failed:', (err && err.message) || err);
            }
        }
        ...
        function applyCanvasState(state) {
            ...
            // If this session has no explicit URL canvas id and current key is empty,
            // recover the most recently edited local canvas snapshot.
