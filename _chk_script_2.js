        ...
        async function restoreCanvas() {
            ...
            // 1. Try IndexedDB
            try {
                const idbRecord = await loadCanvasFromIndexedDb(canvasId);
                if (idbRecord) {
                    restoredState = idbRecord;
            ...
            // 3. Try Supabase (Cloud) - CRITICAL FOR PUBLISHED/SHARED VIEWS
            if (!restoredState && supabaseClient) {
                try {
                    console.log('Attempting Supabase restore for:', canvasId);
                    ...
                    document.addEventListener('DOMContentLoaded', () => {
                        ...
                        // Setup Save & Share / Publish button (Enforce Publish Logic)
                        const saveShareBtn = document.getElementById('saveShareBtn');
                        if (saveShareBtn) {
                            // Ensure button is labeled Publish and opens the modal
                            saveShareBtn.innerHTML = '<i class="fas fa-paper-plane" style="margin-right:8px"></i> Publish';
                            if (typeof openPublishModal === 'function') {
                                saveShareBtn.onclick = (e) => {
                                    e.preventDefault();
                                    openPublishModal();
                                };
                            } else {
                                console.warn('openPublishModal not defined, falling back to previewPage');
                                saveShareBtn.onclick = previewPage;
                            }
                        }
